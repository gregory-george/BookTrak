...
- **Finding the ASIN (the one rough edge).** audnexus is enrichment-by-ASIN; it isn't a great title search. Two supported paths:
  ...
  3. **Title→author discovery (best-effort).** Implemented in the `BookTrak.Audible` namespace: it queries Audible's unofficial catalog-search JSON endpoint (`api.audible.com/1.0/catalog/products?keywords=<title author>`, us region, `response_groups=product_desc,contributors`) to turn a book's title+primary author into candidate ASINs, then routes the chosen ASIN through the normal audnexus enrichment path above. This is a **deliberate narrow carve-out** to the "don't scrape Audible" rule — it uses Audible's own JSON app API (the same one audnexus calls server-side), not HTML scraping, sends no bot-shaped User-Agent, and is purely additive: single adds auto-attach only one high-confidence match (strict title+author gate, `AudiobookMatch`), bulk "Add All" skips it, and the book detail page exposes a "Find matching audiobook" picker. Manual ASIN entry (paths 1–2) remains the reliable fallback whenever search is empty/unavailable.
